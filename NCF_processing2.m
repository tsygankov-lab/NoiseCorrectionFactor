...
% If you use the included data for the first time, start with the default parameters (these are the parameters used for Figure 9 in the paper).
% Notice that this script uses an optimization which is based on the cost function defined by Equation 6 in the paper.  
...
outMASK = imdilate(MASK,SE) - MASK;

for i = 1:length(v)

    R_ncf = (FRET - v(i))./CFP;
    cost(i) = sqrt(mean((R_ncf(outMASK==1) - lev).^2));
...
s2 = subplot(2,3,2);
hold on;
imagesc(MASK+2*outMASK);
plot([0.5 size(MASK,2)+0.5],[Y Y],'r');
axis equal; axis ij; %axis off;
title(['Outside region ' num2str(off_edge) ' pixels away from the edge']);
